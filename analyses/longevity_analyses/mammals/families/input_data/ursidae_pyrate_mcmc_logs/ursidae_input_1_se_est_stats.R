	...
	setwd("/Users/tobias/Documents/Sweden/PhD/rise_and_fall/longevity_analyses/mammals/families/ursidae_pyrate_mcmc_logs")
	tbl = read.table(file = "ursidae_input_1_se_est_stats.txt",header = T)
	pdf(file='fursidae_input_1_se_est_stats.pdf',width=12, height=9)
	time = -tbl$time
	par(mfrow=c(2,2))
	library(scales)
