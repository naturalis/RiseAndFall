...
# creates a table of NCBI taxonomy identifiers (the taxa table).
  ...
  if [ ! -e species.tsv ]; then
    smrt taxize -r Perissodactyla,Camelus -b
  fi 
...
export SUPERSMART_CLADE_MAX_DISTANCE="0.6"
export SUPERSMART_CLADE_MIN_DENSITY="0.1"
